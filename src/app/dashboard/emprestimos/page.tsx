...
import { ArrowRightLeft, BookCheck, Calendar, ChevronDown, ChevronUp, Search, X, CheckCircle2 } from "lucide-react"
...
export default function EmprestimosPage() {
  ...
  // Estado para solicitações pendentes
  ...
  const loadPendingReservations = useCallback(async () => {
    ...
    const result = await getPendingBookReservations();
    if (result.success) {
  ...
  }, [loadActiveLoans, loadPendingReservations]);
  ...
  // Funções para lidar com solicitações de reserva
  const handleApproveReservation = async (id: string) => {
    const result = await approveReservation(id);
    
    if (result.success) {
      toast.success("Reserva aprovada e empréstimo criado com sucesso!");
      loadPendingReservations();
      loadActiveLoans();
    } else {
      toast.error(result.error || "Erro ao aprovar reserva");
    }
  };
  ...
  // Filtra as solicitações de reserva
  const getFilteredReservations = () => {
    if (!reservationFilterText) return pendingReservations;
    
    const searchTerm = reservationFilterText.toLowerCase();
    return pendingReservations.filter(reservation => 
      reservation.book.title.toLowerCase().includes(searchTerm) || 
      reservation.book.author.toLowerCase().includes(searchTerm) || 
      reservation.book.isbn.toLowerCase().includes(searchTerm) || 
      reservation.student.name.toLowerCase().includes(searchTerm) || 
      reservation.student.enrollment.toLowerCase().includes(searchTerm)
    );
  };
                  ...
                  <TabsList className="mb-2">
                    ...
                    <TabsTrigger value="reservations" className="relative">
                      ...
                      {pendingReservations.length > 0 && (
                        <Badge className="ml-1 bg-primary">{pendingReservations.length}</Badge>
                      )}
                    </TabsTrigger>
                  </TabsList>
                  ...
                  {activeTab === "reservations" && (
                    <div className="relative flex-1 mt-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => setReservationFilterText(e.target.value)}
                      />
                    </div>
                  )}
                </Tabs>
                ...
                {activeTab === "reservations" && (
                  <div>
                    {isLoadingReservations ? (
                      <p className="text-center text-muted-foreground">Carregando solicitações...</p>
                    ) : pendingReservations.length === 0 ? (
                      <p className="text-center text-muted-foreground">Nenhuma solicitação pendente.</p>
                    ) : (
                      <div className="space-y-4">
                        <div className="grid grid-cols-6 gap-4 text-sm font-medium text-muted-foreground">
                        ...
                        <div className="max-h-[500px] space-y-3 overflow-auto">
                          {getFilteredReservations().length === 0 ? (
                            <p className="text-center text-muted-foreground py-4">Nenhuma solicitação encontrada com este filtro.</p>
                          ) : (
                            getFilteredReservations().map((reservation) => (
                              <div key={reservation.id} className="grid grid-cols-6 items-center gap-4 text-sm">
                                <div className="col-span-2">
                                  <div className="font-medium">{reservation.book.title}</div>
                                  <div className="text-xs text-muted-foreground">
                                    {reservation.book.author} • ISBN: {reservation.book.isbn}
                                  </div>
                                </div>
                                <div>
                                  <div className="font-medium">{reservation.student.name}</div>
                                  <div className="text-xs text-muted-foreground">
                                    Matrícula: {reservation.student.enrollment}
                                  </div>
                                </div>
                                <div className="text-muted-foreground">
                                  {new Date(reservation.orderDate).toLocaleDateString('pt-BR')}
                                </div>
                                <div>
                                  {new Date(reservation.returnDueDate).toLocaleDateString('pt-BR')}
                                </div>
                                <div className="flex space-x-1">
                                  <Button
                                    variant="outline"
